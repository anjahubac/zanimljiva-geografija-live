...
| Automated by       | `tests/integration/opponent-left.test.ts` — a real socket drop carried through the real reducer into real markup — plus the rendered-output cases in `tests/unit/answer-sheet.test.ts` and the derivation cases in `tests/unit/game-state.test.ts`                                                                                                                    |
...
| After controlled change | 2026-09-23 | _hash recorded in the following commit_ | `npm run verify`                  | pass (3) | pass (7) | pass (16)     | **pass** | 275 tests, 20 files. E1–E3 unchanged in count and outcome. E4 verified in both directions: with the source reverted to baseline 11 of the 14 new tests fail, all pass with the fix. Client-only change; no server, schema, event, timing or scoring file touched. |
